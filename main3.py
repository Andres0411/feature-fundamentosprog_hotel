...
def registrar_operacion(operacion, numero_habitacion):
...
def reservar():
    ...
    for h in habitaciones:
        if h["No"] == numero:
            if h["Estado"] == 1:
                h["Estado"] = 2
                registrar_operacion("Reserva", numero)
...
def checkin():
    ...
    for h in reservadas:
        if h["No"] == numero:
            h["Estado"] = 3
            registrar_operacion("Check-in", numero)
...
def checkout():
    ...
    for h in ocupadas:
        if h["No"] == numero:
            h["Estado"] = 4
            registrar_operacion("Check-out", numero)
...
def mantenimiento():
    ...
    for h in sucias:
        if h["No"] == numero:
            h["Estado"] = 1
            registrar_operacion("Limpieza", numero)
...
def ver_registro_operaciones():
    if not registro_operaciones:
        ...
        for r in registro_operaciones:
            print(f'{r["Fecha"]} - {r["Operacion"]} - Habitación {r["Habitacion"]}')
