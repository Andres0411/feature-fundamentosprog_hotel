    ...
    for h in habitaciones:
        if h["No"] == numero:
            if h["Estado"] == 1:
                ...
                dias_str = input("Ingrese la cantidad de días de reserva: ").strip()
...
def ver_titulares():
    ...
    for t in registro_titulares:
        ...
        print(f'Proceso #{t["ProcesoID"]} - Habitación {t["Habitacion"]} - {t["Nombre"]} (DNI: {t["DNI"]}) - Estado: {estado} - Días: {t["Dias"]} - Total: S/. {t["Subtotal"]} - Fecha: {t["Fecha"]}')

def consultar_operaciones_por_habitacion():
    numero = input("\nIngrese el número de habitación a consultar: ")
    if not numero.isdigit():
        print("Entrada inválida.")
        return
    numero = int(numero)
    operaciones = [op for op in registro_operaciones if op["Habitacion"] == numero]
    if not operaciones:
        print(f"No hay operaciones registradas para la habitación {numero}.")
        return
    print(f"\nOperaciones realizadas para la habitación {numero}:")
    for op in operaciones:
        print(f'{op["Fecha"]} - {op["Operacion"]} - Proceso #{op["ProcesoID"]}')

def consultar_estado_actual_de_habitacion():
    numero = input("\nIngrese el número de habitación a consultar su estado actual: ")
    if not numero.isdigit():
        print("Entrada inválida.")
        return
    numero = int(numero)
    habitacion = next((h for h in habitaciones if h["No"] == numero), None)
    if not habitacion:
        print("Habitación no encontrada.")
        return

    estado = estados.get(habitacion["Estado"], "Desconocido")
    print(f"\nEstado actual de la habitación {numero}: {estado}")

    if habitacion["Estado"] in [2, 3]:  # Reservado u Ocupado
        titular = next((t for t in registro_titulares if t["Habitacion"] == numero), None)
        if titular:
            print(f"Titular actual: {titular['Nombre']} (DNI: {titular['DNI']})")
        else:
            print("Titular no registrado.")

def ver_total_ingresos_checkouts():
    procesos_checkout = []
    for op in registro_operaciones:
        if op["Operacion"] == "Check-out":
            if op["ProcesoID"] not in procesos_checkout:
                procesos_checkout.append(op["ProcesoID"])

    
    total = 0
    for t in registro_titulares:
        if t["ProcesoID"] in procesos_checkout:
            total += t["Subtotal"]

    print(f"\n Total de ingresos (solo procesos con check-out): S/. {total}")
...
def menu():
    while True:
        ...
        print("8. Consultar operaciones por habitación")
        print("9. Consultar estado actual de habitación")
        print("10. Ver total de ingresos (Check-outs)")
        print("11. Salir")
        ...
        if opcion == 0:
            ...
            consultar_operaciones_por_habitacion()
        elif opcion == 9:
            consultar_estado_actual_de_habitacion()
        elif opcion == 10:
            ver_total_ingresos_checkouts()
        elif opcion == 11:
